...
import { createContextHook, useCoupon, useProduct } from "./CartPage.tsx"
...
const [AdminProvider, useAdmin] = createContextHook(() => {
...
function CouponView({ key, coupon }: { key: Key; coupon: Coupon }) {
  return (
    <div key={key} data-testid={`coupon-${key + 1}`} className="bg-gray-100 p-2 rounded">
...
function CouponManageView() {
  const { newCoupon, setNewCoupon, coupons, setCoupons } = useCoupon()
        ...
        <div>
          ...
          <div className="space-y-2">
            {coupons.map((coupon, index) => (
              <CouponView key={index} coupon={coupon} />
            ))}
          </div>
        </div>
      </div>
...
function ProductDiscountView({ discount }: { discount: Discount }) {
                      ...
                      <div>
                        ...
                        <div>
                          ...
                          {editingProduct.discounts.map((discount, index) => (
                            <ProductDiscountView key={index} discount={discount} />
                          ))}
